...
static int bdx_start(struct bdx_priv *priv, int bLoadFw)
{
	...
	if ((priv->state & BDX_STATE_STARTED) == 0) {
		...
		do {
			...
			bdx_rx_alloc_buffers(priv, priv->rxdb0,
					     &priv->rxf_fifo0);
			if (request_irq
	...
	while (dno > 0) {
		if (likely(paged_buffers)) {
			...
			if (unlikely(page_off < 0)) {
				page_size =
				    MIN(LUXOR__MAX_PAGE_SIZE, dno * buf_size);
				page_off =
				    ((page_size / buf_size) - 1) * buf_size;
				gfp_mask = GFP_ATOMIC | __GFP_NOWARN;
				page_order = get_order(page_size);
				buf_order = get_order(buf_size);
				page_order =
				    bdx_get_order(page_order, buf_order);
				...
				while ((page_order >= 0)
					 ...
					 alloc_pages(gfp_mask,
						     page_order)) == NULL)) {
					pr_debug("order %d -> %d", page_order,
						 page_order - 1);
					page_order =
					    bdx_get_order(page_order - 1,
							  buf_order);
				}
				if (page == NULL) {
					pr_err
				...
				dma =
				    pci_map_page(priv->pdev, page, 0L,
...
/*
            pr_debug("SKB: len=%d data_len=%d, truesize=%d head=%p "
