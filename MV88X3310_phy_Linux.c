...
int MV88X3310_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	...
	pr_debug("MV88X3310 ecmd->cmd=%x\n", ecmd->cmd);
	pr_debug("MV88X3310 speed=%u\n", speed);
	pr_debug("MV88X3310 ecmd->autoneg=%u\n", ecmd->autoneg);
	if (priv->autoneg == AUTONEG_ENABLE) {
		priv->advertising =
		    (ADVERTISED_10000baseT_Full | ADVERTISED_1000baseT_Full |
		     ADVERTISED_100baseT_Full | ADVERTISED_Autoneg |
		     SUPPORTED_TP | ADVERTISED_Pause);
		...
		case 5000:	/*5G */
			priv->advertising = (	/* ADVERTISED_5000baseT_Full | */
						    ADVERTISED_Pause);
			break;

		case 2500:	/*2.5G */
			priv->advertising = (	/* ADVERTISED_10000baseT_Full | */
						    ADVERTISED_Pause);
			break;
