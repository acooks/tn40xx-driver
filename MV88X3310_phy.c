				...
				unsigned short phy)
{
	...
	if (priv->deviceId == 0x4027) {
		...
		phy_firmware_size =
		    sizeof(MV88X3310_phy_firmware) / sizeof(u16);
	} else {
		phy_firmware = MV88E2010_phy_firmware;
		phy_firmware_size =
		    sizeof(MV88E2010_phy_firmware) / sizeof(u16);
	...
	do {
		...
		if ((phy_firmware[4] != swab16(~expected_checksum))
		    || (swab16(~val) != swab16(~expected_checksum))) {
